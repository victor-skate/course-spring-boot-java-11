package system.developer.course.services;

import java.util.List;
import java.util.Optional;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import system.developer.course.entities.Category;
import system.developer.course.repositories.CategoryRepository;

@Service // ESSA ANOTAÇÃO INDICA AO SPRING QUE ESSA ENTIDADE
//É UMA DEPENDENCIA A SER INJETADA EM ALGUMA OUTRA ENTIDADE
public class CategoryService {

	@Autowired // A CLASSE CategoryRepository RECEBEU UMA ANOTAÇÃO INDICANDO QUE ELA É UMA
	// DEPENDENCIA A SER INJETADA, E A ANOTAÇÃO Autowired NESSA INSTANCIA DO
	// CategoryRepository
	// FAZ A INJEÇÃO DE DEPENDENCIA AUTOMATICAMENTE
	private CategoryRepository repository;

	public List<Category> findAll() {
		return repository.findAll();
	}

	public Category findById(Long id) {
		Optional<Category> optional = repository.findById(id);
		return optional.get(); // VAI RETORNAR O OBJETO DO OPTIONAL, QUE NESSE CASO É DO TIPO Category

	}

}
